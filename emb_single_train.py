...
from emb_modeling import StaticModel
...
def train(dataset, model, train_config, eval_config, model_config):
    ...
    for ep in train_iterator:
        for step, batch in enumerate(dataLoader):
            ...
            model.zero_grad()
        if ep % 100 == 0:
            torch.save(model.state_dict(), 'checkpoint/params_single' + str(ep) + '.bin')
            model_config['is_traing'] = False
            eval_model = StaticModel(**model_config)
            eval_model.load_state_dict(torch.load('checkpoint/params_single' + str(ep) + '.bin', map_location='cpu'))
            evaluate(eval_model, dataset, eval_config, ep)


def evaluate(model, dataset, eval_config, epoch):
    with open('log_single/result_log.txt', 'a') as f:
    ...
    for step, batch in enumerate(eval_iterator):
        ...
        x_bow = x_bow.to('cpu')
        model.eval()
        mu = model.vae(x_bow)[1].detach().numpy()  # out, mu, var, z, x
        all_out_list.extend(mu)
        accusation_list.extend(accusation)

    # test case accuracy
    set_accusation = list(set(accusation_list))
...
if __name__ == '__main__':
    ...
    model = StaticModel(**model_config)
